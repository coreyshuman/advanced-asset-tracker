...
bool LIS3DH::setupLowPowerWakeMode(uint8_t movementThreshold) {
	...
	//writeRegister8(REG_CTRL_REG1, 0x07); // cts debug
	...
	//writeRegister8(REG_CTRL_REG4, 0x88); // cts debug
...
void LIS3DH::read() {
	...
	req[0] = SPI_READ | SPI_INCREMENT | REG_OUT_X_L;
	req[1] = req[2] = req[3] = req[4] = req[5] = req[6] = 0;
...
int16_t LIS3DH::readXYZmagnitude(){
    read();
